...
import { getSessionId } from '@/utils/sessionIdUtil';
...
export async function addToCart(variationId: number, quantity: number) {
  try {
    ...
    const response = await postData(API_ENDPOINTS.ADD_CART_ITEM, payload);
    return response;
  } catch (error) {
...
export async function getCart() {
  try {
    const token = localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
    let url = API_ENDPOINTS.GET_CART_ITEMS;
    
    if (!token) {
      // Guest user - include sessionId as query param
      const sessionId = getSessionId();
      url = `${API_ENDPOINTS.GET_CART_ITEMS}?sessionId=${sessionId}`;
    }
...
export async function updateCartItem(cartItemId: number, quantity: number) {
  try {
    ...
    const response = await putData(API_ENDPOINTS.UPDATE_CART_ITEM, payload);
    return response;
  } catch (error) {
    return Promise.reject(error);
  }
...
export async function removeFromCart(cartItemId: number) {
  try {
    const token = localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
    let url = API_ENDPOINTS.REMOVE_CART_ITEM.replace(':id', cartItemId.toString());
...
export async function clearCart() {
  try {
    const token = localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
    let url = API_ENDPOINTS.CLEAR_CART;
...
export async function getCartCount() {
  try {
    const token = localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
    let url = API_ENDPOINTS.CART_COUNT;
